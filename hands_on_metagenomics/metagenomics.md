...
'
```
Next We will use the predicted protein sequences (.faa files) generated by Prodigal as input for KofamScan, which performs functional annotation by assigning KEGG Orthologs (KOs) to the proteins. 

Here, each member of the team can select a different MAG to perform the annotation.
(example using MAG 1 from sample NICE3):
```
exec_annotation \
  -o nice3_1_kofam_results.tsv \
  --profile /root/databases/kofam/profiles/prokaryote.hal \
  --ko-list /root/databases/kofam/ko_list \
  --cpu 30 \
  --format detail-tsv \
  nice3_1.proteins.faa
```
Next, we need to convert the results (tsv table) into a file that is readable and ready to upload to [KEGG mapper reconstruct](https://www.genome.jp/kegg/mapper/reconstruct.html).
```
